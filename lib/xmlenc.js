...
function encryptKeyInfoWithScheme(symmetricKey, options, scheme, callback) {
  if(scheme === "AES-128-KW") {
    ...
    keystore.generate('EC', 'P-256').then(function (result) {
      var privateKey = result.toJSON(true);
      privateKey.x = util.base64url.decode(privateKey.x);
      privateKey.y = util.base64url.decode(privateKey.y);
      privateKey.d = util.base64url.decode(privateKey.d);
      ...
      var promise = algorithms.encrypt('ECDH-ES+A128KW', spk, symmetricKey, props)
      promise = promise.then(function(result) {
      var pem = JWK.EC.config.convertToPEM(omit(epk, 'd'), false).replace("-----BEGIN PUBLIC KEY-----", "").replace("-----END PUBLIC KEY-----", "").trim();
        
      //hardcoded for now
      var params = {
        publicKey: JSON.stringify(pem).replace(/"/g, ""),
